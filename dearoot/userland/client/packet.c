...
int tcp(char *srcip, char *dstip, unsigned int srcport, unsigned int dstport, char *data, unsigned int data_len)
{
	...
	if (nbytes > 0) {
		fprintf(stdout, "%s Activate package with TCP: %u bytes was sent!\n", good, nbytes);
		ret = EXIT_SUCCESS;
	}
...
int icmp(char *srcip, char *dstip, char *data, unsigned int data_len)
{
	...
	if (nbytes > 0) {
		fprintf(stdout, "%s Activate package with ICMP: %u bytes was sent!\n", good, nbytes);
		ret = EXIT_SUCCESS;
	}
...
int udp(char *srcip, char *dstip, unsigned int srcport, unsigned int dstport, char *data, unsigned int data_len)
{
	...
	if (nbytes > 0) {
		fprintf(stdout, "%s Activate package with UDP: %u bytes was sent!\n", good, nbytes);
		ret = EXIT_SUCCESS;
	}
